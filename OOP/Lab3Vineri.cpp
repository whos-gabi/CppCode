...
public:
    ...
    {
       out << "Zona: " << this->zona << endl;
       out << "Nume sala: " << this->numeSala << endl;
       out << "Manager sala: " << this->managerSala << endl;
       out << "Numar locuri: " << this->nrLocuri << endl;
       for(int i = 0; i < nrLocuri; i ++)
            if(this->locuriOcupate[i] == 0)
                out << "Locul " << i << " este liber" << endl;
            else
                out << "Locul " << i << " nu este liber" << endl;
       out << "Disponibilitate: " << this->disponibilitate << endl;
    }
...
friend SalaCinema operator+(string s, SalaCinema obj){
    ...
    SalaCinema& operator = (const SalaCinema& obj)
    {
        ...
        this->managerSala = obj.managerSala;
        this->nrLocuri = obj.nrLocuri;
        this->locuriOcupate = new int[obj.nrLocuri];
        for (int i = 0; i < obj.nrLocuri; i++)
        {
            this->locuriOcupate[i] = obj.locuriOcupate[i];
        }
...
float CalcRed(string a){
    if(a=="Eduard")
    return 20;
    return 15;
}
float CalcRed(string a, int c){
    if(a=="Eduard")
    return 20*c;
    return 15*c;
}
